...
export async function createPilot(env,client=createDbk(env)){
 ...
 if(env.RENDER&&!env.RENDER_DISK_MOUNT_PATH)throw Error('Na Render wymagany jest trwały dysk.');
 const dir=resolve(env.PILOT_DATA_DIR),file=resolve(dir,'pilot.json');
 if(env.RENDER&&dir!==resolve(env.RENDER_DISK_MOUNT_PATH)&&!dir.startsWith(resolve(env.RENDER_DISK_MOUNT_PATH)+'/'))throw Error('Dane muszą znajdować się na trwałym dysku.');
 ...
 const publicState=()=>({vehicle:state.vehicle,busy,lastError,orders:state.orders.map(({samples,...o})=>({...o,analysis:analyse({...o,samples})}))});
 async function tick(){
  ...
  }catch{lastError='Pobranie lub zapis nie powiodły się. Następna próba za minutę; dane dostępowe nie są wyświetlane.';}
  finally{busy=false;}
 }
