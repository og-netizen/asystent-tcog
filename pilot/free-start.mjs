...
export async function createFreePilot(env,dbk){
 ...
 try{
  ...
  const server=http.createServer((req,res)=>{
   ...
   if(path==='/health'&&req.method==='GET'){res.writeHead(200,{'Content-Type':'application/json','Cache-Control':'no-store'});return res.end(JSON.stringify({status:'running',pilot:'manual-temporary',version:'0.3.1'}));}
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
 ...
 app.server.listen(Number(process.env.PORT||10000),'0.0.0.0',()=>console.log('TCOG 0.3.1: pilot testowy, zapis tymczasowy, pobieranie wyłącznie ręczne.'));
